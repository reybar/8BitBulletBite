...
public class ShootingNET : NetworkBehaviour {
    ...
    public void CmdShoot(Vector2 firePoint, Vector2 direction, float rotation) {
        ...
        Physics2D.IgnoreCollision(pallet.GetComponent<Collider2D>(), weap.GetComponent<Collider2D>());
        ...
        //TODO destroy at edge of screen, see bullet destroy
    }
    
    [Command]
    public void CmdThrow( Vector2 firePoint ,Vector2 direction, float rotation) {
        if(pickup.weapon1.activeSelf) {
            pickup.slot1 = 0;
        }
        else if(pickup.weapon2.activeSelf) {
            pickup.slot2 = 0;
        }
        ...
        Physics2D.IgnoreCollision(thrown.GetComponent<Collider2D>(), weap.GetComponent<Collider2D>());
